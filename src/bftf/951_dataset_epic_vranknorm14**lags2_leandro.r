...
palancas$version  <- "vranknorm14**lags2_leandro"   #Muy importante, ir cambiando la version
...
#------------------------------------------------------------------------------
#Rankeo las columnas de valor (todas excepto la de foto mes y clase_ternaria)
...
CanaritosImportancia  <- function( dataset )
{
  ...
  entrenamiento  <-  dataset[ , foto_mes>= 201911 &  foto_mes<= 202010 &  foto_mes!=202006 & ( clase01==1 | azar < 0.10 ) ]
...
correr_todo  <- function( palancas )
{
  ...
  cols_analiticas <- colnames(dataset)
  cols_analiticas <- cols_analiticas[4:length(cols_analiticas)-1]
