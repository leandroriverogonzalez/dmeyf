...
palancas$version  <- "vranknormrandom14**_leandro"   #Muy importante, ir cambiando la version
...
Rango  <- function( dataset, cols )
{
  ...
  dataset <- dataset[ , paste0( cols, "_rango") := lapply( .SD, frankv, na.last="keep", ties.method="random" ), 
                      by= foto_mes,
                      .SDcols= cols]
